...
export default abstract class ProxyBase {
    ...
    public add(element: GraphicElement) {
        if (element instanceof GraphicPoint) this.point(element);
        else if (element instanceof GraphicRectangle) this.rectangle(element);
        else if (element instanceof GraphicFilledRectangle)
            this.filledRectangle(element);
        else this.unknownElement(element);
    }
