...
test("should leave number as is", () => {
    expect(prec(1)).toEqual("1");
});

test("should leave number as is", () => {
    expect(prec(1.1)).toEqual("1.1");
});
...
test("should keep only two digits after dot", () => {
    expect(prec(1.12345)).toEqual("1.12");
});

test("should keep only one digit after dot", () => {
    expect(prec(1.10345)).toEqual("1.1");
});

test("should return 0", () => {
    expect(prec(0.0001234)).toEqual("0");
});
...
test("should keep only two digits after dot and also keep sign", () => {
    expect(prec(-123.123)).toEqual("-123.12");
});
